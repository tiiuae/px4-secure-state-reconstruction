...
public:
	SensorMatrixDownsampler() : Node("sensor_matrix_downsampler")
	{
		...
		sensor_matrix_publisher_ = this->create_publisher<px4_offboard_control::msg::TimestampedArray>("/sensor_matrices", 10);
...
private:
	...
	rclcpp::Publisher<px4_offboard_control::msg::TimestampedArray>::SharedPtr sensor_matrix_publisher_;
...
void SensorMatrixDownsampler::timerCallback()
{
    ...
    output.header.stamp.nanosec = static_cast<unsigned int>(static_cast<unsigned long>(ekf_1.timestamp)*1000 - static_cast<unsigned long>(output.header.stamp.sec)*1000000000);
